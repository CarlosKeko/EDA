...
string token(const string &s, char separador, bool cometes, long &primer, long &ultim) {
    ...
    if (!cometes || s[primer] != '"') { // No volem tenir en compte les " o no comen�a per "
    ...
    } else { // comen�a per " i les volem tenir en compte com delimitadors
        primer++;
        ultim = s.find('"', primer);
        ...
        if (ultim == string::npos)
            ...
            if (primer > s.length())
                ultim = string::npos; //era l'�ltim token
        }
    }
    return t;
}
